...
results_path = "{cwd_path}\{current_date}\one_layer".format(cwd_path=str(os.getcwd()), current_date=current_date)
...
configuration_path = "sample_size_{sample_size}sample_dimension_{sample_dimension}neurons{neurons}_activation_type_{activation_type}".format(
    ...
    neurons=configuration_parameters["model"]["number_of_neurons"],
    activation_type=configuration_parameters["model"]["activation_type"])
configuration_parameters_text = open(
    "{results_path}\{configuration_path}configuration_parameters.txt".format(
        results_path=results_path, configuration_path=configuration_path), "w")
configuration_parameters_text.write(str(configuration_parameters))
configuration_parameters_text.close()

f_1.savefig("{results_path}\{configuration_path}_train_error.png".format(
    results_path=results_path, configuration_path=configuration_path))
f_2.savefig("{results_path}\{configuration_path}_test_error.png".format(
    results_path=results_path, configuration_path=configuration_path))
f_3.savefig(
    "{results_path}\{configuration_path}_cost_error.png".format(
        results_path=results_path, configuration_path=configuration_path))